...
try:
    ...
    # Fallback to original if new files not found
    from varken.dbmanager import DBManager
    from varken.iniparser import INIParser
    MULTI_DB_ENABLED = False
    print("✗ Using original single-database mode")
...
if __name__ == "__main__":
    ...
    CONFIG = INIParser('/opt/Varken/data/varken.ini')

    print("DEBUG CONFIG ATTRS:", sorted(CONFIG.__dict__.keys()))
    print("DEBUG tautulli_servers:", getattr(CONFIG, 'tautulli_servers', 'MISSING'))
    print("Tautulli servers:", getattr(CONFIG, 'tautulli_servers', None))
    print("DEBUG enabled_dbs:", CONFIG.get_enabled_databases())
    print("DEBUG CONFIG attrs:", CONFIG.__dict__.keys())

    if hasattr(CONFIG, 'database_configs'):
        CONFIG.database_configs = CONFIG.get_enabled_databases()
    if MULTI_DB_ENABLED and hasattr(CONFIG, 'database_configs'):
        DBMANAGER = DBManager(CONFIG.get_enabled_databases())
        print(f"✓ Connected to {DBMANAGER.multi_manager.get_backend_count()} database(s)")
    else:
